...
test.describe("Admin things list @admin", () => {
  ...
  test("I can navigate through the list using the pagination @read", async ({ thingPage, page }) => {
    ...
    await page.goto("/admin#/admin/things?page=2");
    await expect(page.locator(".datagrid-body tr")).toHaveCount(nbItemsPerPage);
    await expect(await thingPage.getDefaultThing()).not.toBeVisible();
    await expect(page.getByLabel("pagination navigation").locator("li button.Mui-selected")).toHaveAttribute("aria-label", "Page 2");
    await expect(page.getByLabel("Go to previous page")).toBeEnabled();
  ...
  test("I can sort the list by name @read", async ({ thingPage, page }) => {
    await page.getByText("Name").click();
    await expect(await thingPage.getDefaultThing()).not.toBeVisible();
  });

  test("I can remove a thing from the list @write", async ({ page }) => {
