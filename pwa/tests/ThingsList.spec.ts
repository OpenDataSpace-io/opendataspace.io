...
test.describe("Books list", () => {
  ...
  test("I can navigate through the list using the pagination @read", async ({ thingPage, page }) => {
    // test list display
    await expect(page).toHaveTitle("Things Store");
    ...
    await expect(page.getByTestId("thing").or(page.getByTestId("loading"))).toHaveCount(30);
    await expect(await thingPage.getDefaultBook()).not.toBeVisible();
  ...
  test("I can filter the list @read", async ({ thingPage, page }) => {
    // filter by name
    await thingPage.filter({ name: "Thing" });
    await expect(page).toHaveURL(/\/things\?name=Thing/);
    ...
    await page.getByLabel("Go to next page").click();
    await expect(page).toHaveURL(/\/thing\?page=2$/);
    await expect(page.getByTestId("thing").or(page.getByTestId("loading"))).toHaveCount(30);
    await expect(await thingPage.getDefaultThing()).not.toBeVisible();
    await thingPage.filter({ name: "Thing" });
    await expect(page).toHaveURL(/\/things\?name=Thing/);
